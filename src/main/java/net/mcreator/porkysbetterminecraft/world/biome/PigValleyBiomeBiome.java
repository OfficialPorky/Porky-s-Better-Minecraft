...
import net.mcreator.porkysbetterminecraft.entity.PigSoldierEntity;
import net.mcreator.porkysbetterminecraft.entity.PigPresidentEntity;
import net.mcreator.porkysbetterminecraft.entity.PigFarmerEntity;
import net.mcreator.porkysbetterminecraft.block.PorkerLogBlock;
import net.mcreator.porkysbetterminecraft.block.PorkerLeavesBlock;
...
public class PigValleyBiomeBiome extends PorkysBetterminecraftModElements.ModElement {
	@ObjectHolder("porkys_betterminecraft:pig_valley_biome")
	public static final CustomBiome biome = null;
	public PigValleyBiomeBiome(PorkysBetterminecraftModElements instance) {
		super(instance, 73);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.2f).depth(0.1f).scale(0.2f).temperature(0.7999999999999999f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NETHER).waterColor(-10092289).waterFogColor(-10092289)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.DIRT.getDefaultState(), Blocks.DIRT.getDefaultState())));
			setRegistryName("pig_valley_biome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(PorkerLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(PorkerLeavesBlock.block.getDefaultState()))).baseHeight(7)
			...
			this.addSpawn(EntityClassification.AMBIENT, new Biome.SpawnListEntry(PigPresidentEntity.entity, 10, 1, 1));
			this.addSpawn(EntityClassification.AMBIENT, new Biome.SpawnListEntry(PigSoldierEntity.entity, 20, 3, 4));
			this.addSpawn(EntityClassification.AMBIENT, new Biome.SpawnListEntry(PigFarmerEntity.entity, 10, 2, 6));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == PorkerLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, PorkerLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, PorkerLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == PorkerLogBlock.block.getDefaultState().getBlock()
					|| blockType == PorkerLeavesBlock.block.getDefaultState().getBlock()
